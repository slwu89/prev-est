...
sc_model_cpp_mcmc_n <- nimble::compileNimble(sc_model_mcmc_n, 
                                             project = sc_model_n)
samples <- nimble::runMCMC(mcmc = sc_model_cpp_mcmc_n,
                           niter = 1e5, #nburnin = 1e4,
                           progressBar = TRUE, summary = TRUE)
samples
...
sc_mrp_nimble <- nimble::nimbleCode({
  ...
  sigma_income ~ T(dnorm(mean = 0, sd = coef_prior_scale), min = 0) # zip code
  ...
  for(i in 1:6){
    a_income[i] ~ dnorm(mean = 0, sd = sigma_income)
  }
    ...
    for (i_age in 1:N_age){
      for (i_eth in 1:N_eth){
        for (i_male in 1:N_male){
          ...
          #  ilogit(b[1] + b[2] * (i_male-1) +  
          #           #b[3] * edu_bin[edu_bin_index[i_zip, i_age, i_eth, i_male]] +  
          #           b[3] * x_zip_hisp[i_zip] + 
          #           a_zip[i_zip] + a_eth[i_eth] + a_age[i_age] +
          #           a_zip_eth[i_zip * i_eth] + a_zip_age[i_zip * i_age] + 
          #           a_eth_age[i_eth * i_age] + 
          #           a_zip_male[i_zip * i_male] + a_age_male[i_age * i_male] + 
          #           a_eth_male[i_eth * i_male]# + 
          #a_income[i_income]
          #  )
          
          #p_pop[count_index[i_zip, i_age, i_eth, i_male]] <- mean(tmp_pred[])
          for (i_income in 1:6){
            p2[i_income] <- 
              ilogit(b[1] + b[2] * (i_male-1) +  
                       #b[3] * edu_bin[edu_bin_index[i_zip, i_age, i_eth, i_male]] +  
          ...
          p_pop[count_index[i_zip, i_age, i_eth, i_male]] <- mean(p2[]) # SEAN HERE!!
...
sc_mrp_nsamp <- nimble::runMCMC(mcmc = sc_mrp_model_mcmc_nimble_cpp,niter = 1.1e4,nburnin = 1e3,progressBar = TRUE)
